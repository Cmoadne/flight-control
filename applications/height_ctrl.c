...
//更改6.16     原#define EXP_Z_SPEED  ( 4.0f *my_deathzoom( (thr-500), 50 ) )
#define EXP_Z_SPEED  ( 4.0f *my_deathzoom( (thr-500), 100 ) )
//结束6.16
void Height_Ctrl(float T,float thr)
{	
    //static float wz_speed_t;   更改6.16  注释
...
void height_speed_ctrl(float T,float thr,float exp_z_speed,float h_speed)
{
    ...
    height_thr = LIMIT( 2 * thr , 0, 600 );    //   thr在300以上没有区别

    thr_lpf += ( 1 / ( 1 + 1 / ( 0.5f *3.14f *T ) ) ) *( height_thr - thr_lpf );  //T=0.02
    ...
    wz_speed_pid_v.pid_out = thr_lpf + Thr_Weight *LIMIT((wz_speed_pid.kp *exp_z_speed + wz_speed_pid_v.err + wz_speed_pid_v.err_d + wz_speed_pid_v.err_i),-500,500);
...
void Ultra_Ctrl(float T,float thr)
{
    ...
    else
    {
        ...
        if( exp_height > MAX_HEIGHT_CONTROL )
        {
    ...
    if( ultra_dis_tmp < 2000 )    //一次间距小于10   1cm？   10cm/s
    {
        if( ABS(ultra_sp_tmp) < 100 )
        {
            ultra_speed += ( 1 / ( 1 + 1 / ( 4 *3.14f *T ) ) ) * ( (float)(ultra_sp_tmp) - ultra_speed );  //3.14前数字越大接近越快
        }
        else
        {
    ...
    if( ultra_dis_tmp < 2000 )
    {
        ...
        if( ABS(ultra_dis_tmp - ultra_dis_lpf) < 100 )   //上下两次距离小于10cm
        {
    ...
    //更改6.16   原ultra_ctrl.pid_out = LIMIT(ultra_ctrl.pid_out,-500,500);
...
u16 ultra_expect_height;

void get_ultra_expect_height(void)
{
    ultra_expect_height = MIN_HEIGHT_CONTROL + (CH_filter[4] + 500)/(2000-1000)*(MAX_HEIGHT_CONTROL - MIN_HEIGHT_CONTROL);
}
