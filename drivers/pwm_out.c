...
// 描述   : PWM输出
// 备注   :
// 版本   : V0.0 2017.5.11 初始版本
V0.0 2017.5.11 适用于Cmoadne
207 去掉了通道映射数组  注释了通道映射函数
增加了CMOADNE板子输出顺序
******************************************************************/ 
#include "pwm_out.h"
#include "include.h"
...
#ifdef CMOADNE_BOARD
    TIM8->CCR4 = PWM_RADIO *( pwm_tem[0] ) + INIT_DUTY;				//1	
    TIM8->CCR3 = PWM_RADIO *( pwm_tem[1] ) + INIT_DUTY;				//2
    TIM5->CCR4 = PWM_RADIO *( pwm_tem[2] ) + INIT_DUTY;				//3	
    TIM5->CCR3 = PWM_RADIO *( pwm_tem[3] ) + INIT_DUTY;				//4
#else
    TIM1->CCR4 = PWM_RADIO *( pwm_tem[0] ) + INIT_DUTY;				//1	
    TIM1->CCR3 = PWM_RADIO *( pwm_tem[1] ) + INIT_DUTY;				//2
    TIM1->CCR2 = PWM_RADIO *( pwm_tem[2] ) + INIT_DUTY;				//3	
